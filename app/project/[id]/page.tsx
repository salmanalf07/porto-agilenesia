...
import { getProjectById } from "@/lib/portfolio-crud"
...
interface ProjectData {
  ...
  duration: string
  description: string
  squad: Array<{ name: string; role: string; avatarUrl: string }>
  agilenesiaSquad?: Array<{ name: string; role: string; avatarUrl: string }>
  clients?: {
    id: string
    name: string
    logoUrl?: string
...
export default function ProjectDetailPage({ params }: { params: { id: string } }) {
    ...
    const fetchData = async () => {
      try {
        ...
        const parsedProject: ProjectData = {
          ...projectData,
          products:
            typeof projectData.products === "string" ? JSON.parse(projectData.products) : projectData.products || [],
          galleryImages:
            typeof projectData.galleryImages === "string"
              ? JSON.parse(projectData.galleryImages)
              : projectData.galleryImages || [],
          squad: typeof projectData.squad === "string" ? JSON.parse(projectData.squad) : projectData.squad || [],
          agilenesiaSquad:
  ...
  const galleryImages: GalleryImage[] = project?.galleryImages?.length
    ? project.galleryImages.map((img) => ({
        url: img.url,
        alt: img.alt,
        caption: img.caption || "",
      }))
    : project?.coachingImageUrl
      ? [
          {
      ...
      <main className="container mx-auto p-4 md:p-8 lg:p-12">
        ...
        <FadeInUp>
          <div className="mb-12 md:mb-16 text-center md:text-left">
            ...
            <p className="text-subtitle text-primary font-medium">{project.clients?.name || project.clientName}</p>
          </div>
        </FadeInUp>
        ...
        <div className="grid gap-12 lg:grid-cols-3">
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-20 space-y-8">
              ...
              {/* Agilenesia Squad */}
              {project.agilenesiaSquad && project.agilenesiaSquad.length > 0 && (
                <FadeInUp delay={0.9}>
                  <motion.div
                    whileHover={{ y: -2, scale: 1.01 }}
                    transition={{ type: "spring", stiffness: 300, damping: 30 }}
                  >
                    <Card className="shadow-lg hover:shadow-xl transition-shadow duration-300">
                      ...
                      <CardContent className="space-y-4">
                        <StaggerContainer>
                          {project.agilenesiaSquad.map((member, index) => (
                            <StaggerItem key={member.name}>
                              <motion.div
                                className="flex items-center gap-3 p-3 rounded-md hover:bg-muted/50 transition-colors cursor-pointer"
                                whileHover={{ x: 5, scale: 1.02 }}
